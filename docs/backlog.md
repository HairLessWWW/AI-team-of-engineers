...
### M4-00E. Переделать Telegram UX под дружелюбный выбор специалистов
...
Сделать UX бота похожим на выбор AI-сотрудника, а не на набор slash-команд.

Acceptance criteria:

- `/start` показывает короткое меню без перегруза командами.
- У каждого специалиста есть короткое русское имя, роль и описание.
- Пользователь может выбрать специалиста кнопкой и дальше писать обычным текстом.
- Совещание собирается через выбор участников кнопками.
- Кнопочные переходы редактируют текущее сообщение, чтобы чат не разрастался.
- Старые slash-команды сохранены в `/help`.

### M4-00F. Добавить ролевой фильтр ответов специалистов
...
Ограничить ответы AI-специалистов рамками их инженерной роли.

Acceptance criteria:

- Каждый специалист получает явную границу роли в system prompt.
- Если вопрос вне роли, специалист должен обозначить границу компетенции.
- Специалист предлагает, кого подключить вместо ответа за соседнее направление.
- Ролевые prompt-файлы содержат аналогичное ограничение.

### M4-00G. Добавить память диалогов специалистов
...
Добавить SQLite-память для диалогов пользователя с каждым AI-специалистом.

Acceptance criteria:

- Есть `TELEGRAM_MEMORY_DB`.
- Есть `TELEGRAM_MEMORY_DEPTH`.
- Вопросы и ответы сохраняются в SQLite.
- Память разделена по пользователю и специалисту.
- Последние сообщения подмешиваются в LLM-контекст.
- Есть команды `/memory` и `/forget`.
